...
class _TMapState extends State<TMap> {
  ...
  List<Widget> _buildPolylineLayers(bool isDark) {
    ...
    return [
      PolylineLayer(
        polylines: widget.polylines.map((p) => Polyline(points: p.points, color: casingColor, strokeWidth: p.strokeWidth + 3)).toList(),
      ),
      PolylineLayer(
        polylines: widget.polylines.map((p) => Polyline(points: p.points, color: p.color, strokeWidth: p.strokeWidth)).toList(),
      ),
    ];
  }
    ...
    return [
      PolygonLayer(
        polygons: widget.polygons
            .map(
              (p) => Polygon(points: p.points, color: Colors.transparent, borderColor: shadowColor, borderStrokeWidth: p.borderWidth + 4),
            )
            .toList(),
      ),
      PolygonLayer(
        polygons: widget.polygons
            .map(
              (p) =>
                  Polygon(points: p.points, color: p.color, borderColor: p.borderColor ?? colors.outline, borderStrokeWidth: p.borderWidth),
            )
            .toList(),
      ),
    ];
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ClipRRect(borderRadius: BorderRadius.circular(widget.borderRadius), child: _buildMap(context)),
    );
  }
}
