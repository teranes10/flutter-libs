...
class _TBarcodeScannerState extends State<TBarcodeScanner> with TPopupStateMixin<TBarcodeScanner>, SingleTickerProviderStateMixin {
        ...
        children: [
                            ...
                            return Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(error.errorCode.name, style: context.textTheme.bodyMedium, textAlign: TextAlign.center),
                                ],
                              ),
                            );
                          },
                        ),
                        CustomPaint(painter: _ScannerOverlayPainter(scanWindow: scanWindow, borderRadius: 12)),
                        _ScanningLine(animation: _animationController, scanWindow: scanWindow),
                        if (!kIsWeb)
                          Positioned(
                            top: 8,
                            right: 8,
          ...
          if (widget.mode == TBarcodeScannerMode.stayOpen)
            Padding(
              ...
              child: TButton(text: 'Done', onTap: hidePopup, type: TButtonType.solid),
            ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return buildWithDropdownTarget(
      child: KeyedSubtree(
        key: GlobalKey(),
        child: InkWell(onTap: () => togglePopup(context), borderRadius: BorderRadius.circular(8), child: widget.child),
      ),
    );
  }
}
...
class _ScanningLine extends AnimatedWidget {
  final Rect scanWindow;
  const _ScanningLine({required Animation<double> animation, required this.scanWindow}) : super(listenable: animation);
    ...
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          boxShadow: [BoxShadow(color: Colors.red.withValues(alpha: 0.5), blurRadius: 4, spreadRadius: 2)],
          color: Colors.red,
        ),
      ),
    );
